  ...
  return (
    <Container>
        ...
        <MainContent>
            ...
            <InputSection>
                <TitleInput onBlur = {(e) => {updatePostInfo("title",e.target.value)}} placeholder='채용 공고 제목을 입력해주세요.'/>
                <CaptionInput onBlur = {(e) => {updatePostInfo("companyName",e.target.value)}} placeholder='기업 이름을 입력해주세요'/>
            </InputSection>
            </TitleInputSection>
            <PostInfoTable updateValue = {(tableValue) => {updatePostInfoTable(tableValue)}}/>
            <SearchInput data ={skillData} title ="기술스택(업무 툴/스킬)" placeholder="기술 스택을 등록해주세요." onChange = {(value)=>{updatePostInfo("skills",value)}}/>
            <InputArray title ="주요 업무" placeholder ="주요 업무를 입력해주세요." updateValue = {(value) => {updatePostInfo("jobDuties",value)}}/>
            <InputArray title ="자격 요건" placeholder ="자격 요건을 입력해주세요." updateValue = {(value) => {updatePostInfo("requirements",value)}}/>
            <InputArray title ="우대 사항" placeholder ="우대사항 조건을 입력해주세요." updateValue = {(value) => {updatePostInfo("additionalPreferences",value)}}/>
            <InputArray title ="복지 및 혜택" placeholder ="복지 및 해택을 입력해주세요." mainInput={"기업의 장점에 대해 한줄로 간단하게 작성해주세요."} updateValue = {(value) => {updatePostInfo("employeeBenefits",value)}}/>
            <Divider/>
